...

{
  ...
  overview: "Live portfolio and shop built to grow with the buisness, for an artist soon to start selling original paintings and prints. The site launches as a portfolio with an admin toggle that switches the whole site into a full e-commerce shop or a shop redirect, there are multiple other features built to give the admin freedom in how they want to sell now and in the future.",
  ...
  outcome: "Customers browse and buy once the shop's switched on. Checkout reserves stock the moment it starts, not after payment, closing a race condition that could otherwise let two people buy the last unit of a one-of-a-kind piece at the same time. Guests verify their email with a one-time code before paying, and the postcode and city our checked against live UK data before an order goes through. Every form that collects data (contact, newsletter, registration, checkout) requires its own consent checkbox, checked on the server, not just the browser. Abandoned sign-ups delete themselves after 48 hours, unsubscribing from the newsletter is one click, old orders past the 6-year retention period can be anonymised from the admin panel. The owner runs all of it: shop and archive content, products and stock, legal pages (updates automatically with site mode on mode for shop theres with secondary switch for full ecomerse), registration, contact form, newsletter, from one panel with no code changes needed. Built with GDPR-compliant data handling: a full privacy notice, consent capture at every point data is collected, self-serve password reset and account deletion. Deployed serverlessly on Netlify with a Neon Postgres database.",
  tags: ["Design", "Development", "Deployment"],
},
